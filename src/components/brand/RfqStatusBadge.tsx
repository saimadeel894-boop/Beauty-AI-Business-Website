...
export const RfqStatusBadge = React.forwardRef<HTMLDivElement, RfqStatusBadgeProps>(
  ({ status, ...props }, ref) => {
    const getProps = () => {
      switch (status) {
        case "draft":
    ...
    return (
      <Badge ref={ref} variant="outline" className={className} {...props}>
        {label}
      </Badge>
    );
  }
);
RfqStatusBadge.displayName = "RfqStatusBadge";
